    ...
    <nav aria-label="Design system navigation" className="space-y-5 py-2">
      ...
      {!showOverview && groups.length === 0 ? (
        <p className="px-2 py-4 text-sm text-muted-foreground">
          Geen onderdelen gevonden voor “{query}”.
        </p>
      ) : null}
    </nav>
  );
...
export function DesignSystemBrowser() {
  ...
  return (
    <div className={`momentum-browser min-h-screen bg-background text-foreground md:grid md:grid-cols-[270px_minmax(0,1fr)] ${isDark ? 'dark' : ''}`}>
      <aside className="border-b bg-muted/20 md:sticky md:top-0 md:flex md:h-screen md:flex-col md:border-b-0 md:border-r">
        <div className="flex items-start justify-between gap-3 border-b px-5 py-5">
          <div>
            <p className="font-serif text-base font-semibold tracking-tight">{DESIGN_SYSTEM.title}</p>
            <p className="mt-1 font-mono text-[10px] uppercase tracking-[.12em] text-muted-foreground">focused momentum / v1</p>
          </div>
          <button
            type="button"
            onClick={() => setIsDark((current) => !current)}
            aria-label={isDark ? 'Schakel naar lichte weergave' : 'Schakel naar donkere weergave'}
            aria-pressed={isDark}
            className="grid h-8 w-8 place-items-center rounded-md border bg-card text-muted-foreground transition-colors hover:text-foreground focus-visible:outline-none"
          >
        ...
        <div className="p-4 pb-2">
          <Input
            ...
            aria-label="Zoek in het designsysteem"
            placeholder="Zoek in Momentum…"
          />
        </div>
